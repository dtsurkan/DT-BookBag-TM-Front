...
export const getBooks = () => get(STRAPI_URL, `/books?_sort=created_at:DESC&book_status=added`);

export const getBookBySlug = (slug) => get(STRAPI_URL, `/books?slug=${slug}`);

export const getBookBySellerID = (id) =>
  get(STRAPI_URL, `/books?seller.id=${id}&_sort=created_at:DESC&book_status=added`);
...
export const getSellerEmail = async (slug) =>
  await getBookBySlug(slug).then(({ data }) => data[0].seller);
